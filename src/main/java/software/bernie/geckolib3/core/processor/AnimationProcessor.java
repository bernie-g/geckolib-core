        ...
        for (Map.Entry<String, DirtyTracker> tracker : modelTracker.entrySet()) {
            ...
            if (saveSnapshot == null) {
                if (crashWhenCantFindBone) {
                    throw new RuntimeException(
                            "Could not find save snapshot for bone: " + tracker.getValue().model.getName() + ". Please don't add bones that are used in an animation at runtime.");
                } else {
                    continue;
                }
            }
            ...
            if (!tracker.getValue().hasRotationChanged) {
                ...
                double percentageReset = Math.min(
                        (seekTime - saveSnapshot.mostRecentResetRotationTick) / resetTickLength, 1);
            ...
            if (!tracker.getValue().hasPositionChanged) {
                ...
                double percentageReset = Math.min(
                        (seekTime - saveSnapshot.mostRecentResetPositionTick) / resetTickLength, 1);
